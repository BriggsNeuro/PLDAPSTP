function oritrial_free_side(p,state)
...
if p.trial.stimulus.fullField == 1 
    switch p.trial.side
        case p.trial.stimulus.side.LEFT
             p.trial.gratPos = [0 0 512 768]; % change to your resolution - SZ
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf);
        case p.trial.stimulus.side.RIGHT
             p.trial.gratPos = [512 0 1024 768];
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf)./2;
    end
else
    switch p.trial.side
        case p.trial.stimulus.side.LEFT
             p.trial.gratPos = [0 192 512 576]; % change to your resolution - SZ
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf);
        case p.trial.stimulus.side.RIGHT
             p.trial.gratPos = [512 192 1024 576];
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf)./2;
    end
end
%make grating
