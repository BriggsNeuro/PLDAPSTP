function oritrial_free_contrast(p,state)
...
switch state
    ...
    case p.trial.pldaps.trialStates.frameDraw
        ...
        elseif p.trial.state==p.trial.stimulus.states.PRESTIMON && p.conditions{p.trial.pldaps.iTrial}.centralflash
        ...
        elseif p.trial.state==p.trial.stimulus.states.STIMON || (p.trial.state==p.trial.stimulus.states.INCORRECT && p.trial.stimulus.forceCorrect ~= 0)
            if p.trial.stimulus.midpointIR 
                if ~p.trial.stimulus.midpointCrossed
                    if p.conditions{p.trial.pldaps.iTrial}.centralflash
                        Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexCentral,[],p.trial.gratPosCentral,0); % added since 3/29/2023
                    else
                        Screen(p.trial.display.ptr, 'FillRect', 0.5)
                    end
                elseif p.trial.stimulus.midpointCrossed == 1 && ~isfield(p.trial.stimulus,'timemidpointCrossed')
                        Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexC,[p.trial.jitter_from_edge_px  0 960 + p.trial.jitter_from_edge_px 540],p.trial.gratPosC,0);
                        Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexI,[-p.trial.jitter_from_edge_px  0 960 - p.trial.jitter_from_edge_px 540],p.trial.gratPosI,0);
...
%                             Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexCentral,[],p.trial.gratPosCentral,0);
%                         end
                elseif p.trial.stimulus.pretime_final~=0 && p.trial.ttime >= p.trial.stimulus.timemidpointCrossed + p.trial.stimulus.pretime_final
                        Screen(p.trial.display.ptr, 'FillRect', 0.5)
                        if p.trial.ttime <= p.trial.stimulus.timemidpointCrossed + 2*p.trial.stimulus.pretime_final % no need to do that long time
                            digital_out(p.trial.behavior.optogenetics.channel,0); % turn off opto here
                            digital_out(p.trial.behavior.outputtimestamp.channel,0);
                        end
                else
                    Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexC,[p.trial.jitter_from_edge_px  0 960 + p.trial.jitter_from_edge_px 540],p.trial.gratPosC,0);
                    Screen('DrawTexture',p.trial.display.ptr,p.trial.gratTexI,[-p.trial.jitter_from_edge_px  0 960 - p.trial.jitter_from_edge_px 540],p.trial.gratPosI,0);
%                     Screen(p.trial.display.ptr, 'FillRect', 1,[960;0;980;1024])
%                     Screen(p.trial.display.ptr, 'FillRect', 1,[360;0;380;1024])
...
%                     end
                    ...
                    if p.trial.stimulus.midpointCrossed && p.trial.stimulus.midpointOpto == 1 && p.trial.ttime < p.trial.stimulus.timemidpointCrossed + p.trial.stimulus.pretime_final/5 % no need to do that long time
                    %    if any(strcmp(p.trial.stimulus.step,{'pretime','contrast'}))
                            if mod(p.trial.pldaps.iTrial,2) 
                                %amount=0.1;
                    ...
                    %    else
                            % digital_out(p.trial.behavior.optogenetics.channel,1); % commented out since 9/14/2022
                    %    end
                    end
                end
            else
...
%                 if p.trial.stimulus.midpointOpto == 0
%                     digital_out(p.trial.behavior.optogenetics.channel,1);
%                     digital_out(p.trial.behavior.optogenetics.channel,0);
%                 end       
              %  if ~any(strcmp(p.trial.stimulus.step,{'pretime','contrast'})) % commented out since 09/27/22
              %if p.trial.stimulus.tailbreakmidpoint == 1 % added since 11/30/22
                    digital_out(p.trial.behavior.outputtimestamp.channel,1); % added since 09/14/22
              %end
              %  end
            end
            %note timepoint
            ...
            p.trialMem.rearTimestamp = [p.trialMem.rearTimestamp p.trial.trstart + p.trial.ttime];
            %deliver reward
...
%         end
    ...
    case p.trial.stimulus.states.STIMON %stimulus shown; port selected in response
        %check whether midpoint is broken
        if p.trial.stimulus.midpointIR && any(find(p.trial.ports.status==1) == p.trial.stimulus.port.MIDDLE) %check IR beam for box midpoint
            ...
            %note timepoint
            if ~isfield(p.trial.stimulus,'timemidpointCrossed')  %don't repetitively update timemidpointCrossed
...
%                 if p.trial.stimulus.tailbreakmidpoint == 0 % added since 11/30/22
%                     digital_out(p.trial.behavior.outputtimestamp.channel,1); % added since 11/30/22
%                 end
            end
            p.trialMem.midpointTimestamp = [p.trialMem.midpointTimestamp p.trial.trstart + p.trial.ttime];
        end
        % move stimulus according to jitter_progress
        if length(p.trialMem.locationjitter) >= 2 && isfield(p.trial.stimulus,'framemidpointCrossed') && isfield(p.trial.stimulus,'timemidpointCrossed') && p.trial.ttime - p.trial.stimulus.timemidpointCrossed <= p.trial.stimulus.pretime_final
                number_frame_locationjitter = round(1/p.trial.display.ifi) * 0.27; % *(p.conditions{p.trial.pldaps.iTrial}.pretime + p.conditions{p.trial.pldaps.iTrial}.pretimeadd); changed since 5/26/2023
                p.trialMem.locationjitter_progress = linspace(p.trialMem.locationjitter(1),p.trialMem.locationjitter(end),number_frame_locationjitter);
                if ((p.trial.iFrame - p.trial.stimulus.framemidpointCrossed + 1) <= number_frame_locationjitter)
                    ...
                    p.trial.stimulus.locationjitter = p.trialMem.locationjitter_progress(end); 
                end
                switch p.trial.stimulus.LeftRight
                    % addded (1+(p.trial.stimulus.tf_final>0) since 3/29/2023, because when tf >0, somehow locationjitter needs t be doubled to achieve the same distance of movement
                    case 1 % correct choice on the left
                         p.trial.gratPosC = [(0 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) 0 (p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) p.trial.display.screenSize(4)]; % change to your resolution - SZ
                         p.trial.gratPosI = [(p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) 0 (p.trial.display.screenSize(3) + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) p.trial.display.screenSize(4)];
                        % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf);
                    case 2 % correct choice on the right
                         p.trial.gratPosC = [(p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) 0 (p.trial.display.screenSize(3) + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) p.trial.display.screenSize(4)];
                         p.trial.gratPosI = [(0 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) 0 (p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter *(1+(p.trial.stimulus.tf_final>0))) p.trial.display.screenSize(4)];
                        % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf)./2;
                end
        elseif length(p.trialMem.locationjitter) >= 2 && isfield(p.trial.stimulus,'framemidpointCrossed') && p.trial.ttime - p.trial.stimulus.timemidpointCrossed > p.trial.stimulus.pretime_final
            display(1)
        end
        % Drift grating
        p.trial.stimulus.tf_final =  p.conditions{p.trial.pldaps.iTrial}.tf +  p.conditions{p.trial.pldaps.iTrial}.tfadd;
        if p.trial.stimulus.tf_final > 0 && isfield(p.trial.stimulus,'framemidpointCrossed') && isfield(p.trial.stimulus,'timemidpointCrossed') && (p.trial.ttime <= p.trial.stimulus.timemidpointCrossed + p.trial.stimulus.pretime_final || p.trial.stimulus.pretime_final == 0)
            % cycle of movement per frame
            tf = p.trial.stimulus.tf_final;
            dCycle = 1/Screen('FrameRate',2) * tf; % 1/(frame/sec) * cycle/sec = cycle/frame
            % phase(degree) of movement per frame
            dPhase = dCycle * 360; %cycle/frame * degree/cycle = degree/frame
            % phase of each frame
            newPhase = mod(p.trial.stimulus.phase + dPhase * (p.trial.iFrame - p.trial.stimulus.framemidpointCrossed + 1),360);
            sdom1=cos(p.trial.removelater.sdom-newPhase*pi/180);
        ...
        elseif p.trial.stimulus.tf_final > 0 && isfield(p.trial.stimulus,'framemidpointCrossed') && isfield(p.trial.stimulus,'timemidpointCrossed') && p.trial.ttime > p.trial.stimulus.timemidpointCrossed + p.trial.stimulus.pretime_final && isfield(p.trial.removelater,'sdom')  && p.trial.stimulus.pretime_final ~= 0
...
%             % if breaking is too short, probably tail break, rescue it
%             if ((p.trial.stimulus.timemidpointCrossedend - p.trial.stimulus.timemidpointCrossed) <= 0.1)  &&  p.trialMem.midpointreset ~=0
...
%         % reset midpoint 1.5 sec after break % commented, 7/31/2022
%         if p.trial.stimulus.midpointIR && isfield(p.trial.stimulus,'timemidpointCrossed') && (p.trial.ttime - p.trial.stimulus.timemidpointCrossed >= 1.5) && (p.trial.ttime - p.trial.stimulus.timemidpointCrossed >= p.trial.stimulus.pretime_final)  &&  p.trialMem.midpointreset ~=0
%             if ~isfield(p.trial.stimulus,'reset')
%                    p.trial.stimulus.reset = [];
%             end
%             p.trial.stimulus.reset = [p.trial.stimulus.reset p.trial.stimulus.timemidpointCrossed];
...
%             disp(['midpoint reset'])
%         end
        % if rear port activated again, rescue midpoint % commented, 7/31/2022
%         if  any(find(p.trial.ports.status==1)==p.trial.stimulus.port.START) 
...
if  ~p.trialMem.staircasepause && p.trial.pldaps.iTrial > p.trialMem.practice % + 3 (Commented out since3/8/2023)
    switch p.trial.stimulus.step
        case {'var','contrast'}
            p.conditions{p.trial.pldaps.iTrial}.select = p.conditions{p.trial.pldaps.iTrial-1}.select;
            % decrease step after three correct trials
            if p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 4 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit','step'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit','step'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'}))...
                ...
                if p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd -p.trial.stimulus.staircasestep >= p.trialMem.loweststep 
                    p.trial.pldaps.step = p.trial.stimulus.staircasestep;
                    p.conditions{p.trial.pldaps.iTrial}.select = p.conditions{p.trial.pldaps.iTrial}.select - p.trial.pldaps.step;
                      % (Commented out since3/8/2023)
...
%                     p.conditions{p.trial.pldaps.iTrial}.radiusadd = 0;
%                     p.conditions{p.trial.pldaps.iTrial+1}.radiusadd = 0;
                    disp('step-1')
                else
                    disp('lowest step')
                end
            % increase step after one incorrect (added since3/8/2023)
            elseif p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 5 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,{'quit'}))...
                    && ~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial')
                if p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd + p.trial.stimulus.staircasestep <= p.trialMem.higheststep
                    p.trial.pldaps.step = p.trial.stimulus.staircasestep;
                    p.conditions{p.trial.pldaps.iTrial}.select = p.conditions{p.trial.pldaps.iTrial}.select +p.trial.pldaps.step;
                    disp('step+1')
                else
...
%             % increase step when there are two incorrect trials in the last three trials (commented out since3/8/2023)
%             elseif p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 5 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,{'quit'}))...
...
%                     && (isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') || isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') || isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,'goodtrial')) % continuous three incorrect doesn't increase staircase
...
%                     disp('step+1')
% %                 elseif p.conditions{p.trial.pldaps.iTrial}.pretime + p.conditions{p.trial.pldaps.iTrial}.pretimeadd < p.trial.stimulus.duration.pretime + 0.1
% %                     disp('highest step')
% %                     p.conditions{p.trial.pldaps.iTrial}.pretimeadd = p.conditions{p.trial.pldaps.iTrial}.pretimeadd + 0.05;
% %                     %p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.05;
%                 else
%                     disp('highest step')
%                 end

%             % level 3 & 1, only one error needed to go up. (commented out since3/8/2023)
%             elseif p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 5 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,{'quit'}))...
%                     && ~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') ... 
...
%             % Restart staircase after three correct trials during practice trials (level 12) (commented out since3/8/2023)
...
%             % Go to practice trial at level 12 after incorrects at level 11(commented out since3/8/2023)
%             elseif p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 5 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'}))&& all(~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,{'quit'}))...
...
%                     && p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd == 11 ...
%                     && any(round((p.trialMem.stats.val(3,:) - p.trialMem.stats.val(4,:))./2,2) == round(p.trial.stimulus.range_var_source(9),2)) % already got to level 9
...
%                     if mod(p.conditions{p.trial.pldaps.iTrial}.radius,1)==0 && p.conditions{p.trial.pldaps.iTrial}.radiusadd ==0 % stay at level 11 first
%                         p.conditions{p.trial.pldaps.iTrial}.radiusadd = 0.5;
%                         p.conditions{p.trial.pldaps.iTrial+1}.radiusadd = 0.5;
%                         p.trial.pldaps.step = 0.5;
%                     elseif ~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') && ~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') ... %(length(find([~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,'goodtrial')])))>=3 % 3 mistakes in last 4 trials
%                             && (isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') || isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') || isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,'goodtrial')) % continuous three incorrect doesn't increase staircase
%                         p.trial.pldaps.step = 1;
%                         p.conditions{p.trial.pldaps.iTrial}.select = p.conditions{p.trial.pldaps.iTrial}.select + 1;
%                         p.conditions{p.trial.pldaps.iTrial}.radiusadd = 0;
    ...
    %        end
        ...
        case 'pretime'
            ...
            elseif p.trialMem.lock ==0 && p.trial.pldaps.iTrial >= 5 && all(~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,{'quit'})) && all(~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,{'quit'}))&& all(~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,{'quit'}))...
                    && ~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') && (~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') || ~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,'goodtrial')|| ~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,'goodtrial')) ... % incorrect + another incorrect in previous two trials
                    && p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd == 11 ...
                    && any(round(p.trialMem.stats.val(5,:),2) == round(p.trial.stimulus.duration.pretime_var_source(9),2)) % already got to level 9
                    if mod(p.conditions{p.trial.pldaps.iTrial}.radius,1)==0 && p.conditions{p.trial.pldaps.iTrial}.radiusadd ==0
                        p.conditions{p.trial.pldaps.iTrial}.radiusadd = 0.5;
                        p.conditions{p.trial.pldaps.iTrial+1}.radiusadd = 0.5;
                        p.trial.pldaps.step = 0.5;
                    elseif ~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') && ~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') ... %(length(find([~isfield(p.data{p.trial.pldaps.iTrial-1}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-2}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-3}.pldaps,'goodtrial') ~isfield(p.data{p.trial.pldaps.iTrial-4}.pldaps,'goodtrial')])))>=3 % 3 mistakes in last 4 trials
...
% pretime also a small staircase when it is not in instruction trial
if p.trialMem.pretimehelp && p.trial.pldaps.iTrial >= 25 && ~any(strcmp(p.trial.stimulus.step,{'pretime'})) && p.trialMem.block==0
   ...
   if totalcorrectrate14 < 0.6 %8/14 = 0.57
        if p.conditions{p.trial.pldaps.iTrial}.pretimeadd - p.conditions{p.trialMem.practice+1}.pretimeadd ==0.01
            p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.06;
            p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.06;
        elseif p.conditions{p.trial.pldaps.iTrial}.pretimeadd == p.conditions{p.trialMem.practice+1}.pretimeadd
            p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.05;
            p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.05;
        end
   elseif totalcorrectrate14 >= 0.6 %9/14 = 0.64
        if p.conditions{p.trial.pldaps.iTrial}.pretimeadd - p.conditions{p.trialMem.practice+1}.pretimeadd ==0.06
            p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.01;
            p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.01;
        elseif p.conditions{p.trial.pldaps.iTrial}.pretimeadd - p.conditions{p.trialMem.practice+1}.pretimeadd ==0.05
           p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0;
           p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0;
        end
   end
elseif p.trialMem.pretimehelp && p.trial.pldaps.iTrial >= 25 && any(strcmp(p.trial.stimulus.step,{'pretime'})) && p.trialMem.block==0
    totalcorrectrate14 = sum(p.trialMem.stats.count.accumC(end-13:end,:),'all')./(sum(p.trialMem.stats.count.accumC(end-13:end,:),'all')+ sum(p.trialMem.stats.count.accumI(end-13:end,:),'all'));
    if totalcorrectrate14 < 0.6 %8/14 = 0.57 
        p.conditions{p.trial.pldaps.iTrial}.range_var = 32.5;
        p.conditions{p.trial.pldaps.iTrial+1}.range_var = 32.5;
   elseif totalcorrectrate14 >= 0.6 %9/14 = 0.64
        p.conditions{p.trial.pldaps.iTrial}.range_var = p.conditions{2}.range_var;
        p.conditions{p.trial.pldaps.iTrial+1}.range_var = p.conditions{2}.range_var;
   end
% % Used before 11/30/22
%     if totalcorrectrate14 < 0.65 && totalcorrectrate14 >= 0.55
%         if mod(p.conditions{p.trial.pldaps.iTrial}.pretimeadd,0.05)==0.01
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.06;
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.06;
%         else
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.05;
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.05;
%         end
%     elseif totalcorrectrate14 < 0.55 && ~any(strcmp(p.trial.stimulus.step,{'contrast'}))
%         if mod(p.conditions{p.trial.pldaps.iTrial}.pretimeadd,0.05)==0.01
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.11;
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.11;
%         else
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.1;
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.1;
%         end
%     elseif totalcorrectrate14 < 0.55 && any(strcmp(p.trial.stimulus.step,{'contrast'}))
%         if mod(p.conditions{p.trial.pldaps.iTrial}.pretimeadd,0.05)==0.01
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.06;
...
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.05;
%         end
%     elseif p.trialMem.pretimehelp && totalcorrectrate14 >= 0.65
%         if mod(p.conditions{p.trial.pldaps.iTrial}.pretimeadd,0.05)==0.01
%             p.conditions{p.trial.pldaps.iTrial}.pretimeadd = 0.01;
%             p.conditions{p.trial.pldaps.iTrial+1}.pretimeadd = 0.01;
%         else
...
%%
p.trial.stimulus.range_mean = p.conditions{p.trial.pldaps.iTrial}.range_mean + p.conditions{p.trial.pldaps.iTrial}.range_meanadd; 
switch p.trial.stimulus.step 
    case {0,'var','contrast'}
        if p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd >= 1
            ...
            p.trial.stimulus.range_var = p.trial.stimulus.range_var_source(p.conditions{p.trial.pldaps.iTrial}.select);
        end
        p.trial.stimulus.radius_final = p.conditions{p.trial.pldaps.iTrial}.radius + p.conditions{p.trial.pldaps.iTrial}.radiusadd;
        p.trial.stimulus.pretime_final = p.conditions{p.trial.pldaps.iTrial}.pretime + p.conditions{p.trial.pldaps.iTrial}.pretimeadd;
    case 'radius'
        if p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd >= 1
            p.trial.stimulus.radius_final = p.trial.stimulus.radius(p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd);
        else
            p.trial.stimulus.radius_final = p.trial.stimulus.radius(p.conditions{p.trial.pldaps.iTrial}.select);
        end
        p.trial.stimulus.range_var = p.conditions{p.trial.pldaps.iTrial}.range_var;
        p.trial.stimulus.pretime_final = p.conditions{p.trial.pldaps.iTrial}.pretime + p.conditions{p.trial.pldaps.iTrial}.pretimeadd;
    case 'pretime'
        if p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd >= 1
            p.trial.stimulus.pretime_final = p.trial.stimulus.duration.pretime_var_source(p.conditions{p.trial.pldaps.iTrial}.select + p.conditions{p.trial.pldaps.iTrial}.range_varadd);
        else
            p.trial.stimulus.pretime_final = p.trial.stimulus.duration.pretime_var_source(p.conditions{p.trial.pldaps.iTrial}.select);
        end
        p.trial.stimulus.range_var = p.conditions{p.trial.pldaps.iTrial}.range_var;
        p.trial.stimulus.radius_final = p.conditions{p.trial.pldaps.iTrial}.radius + p.conditions{p.trial.pldaps.iTrial}.radiusadd;
end
...
p.trial.stimulus.rangeC = p.trial.stimulus.range_mean + p.trial.stimulus.range_var; % C: correct choice, I: incorrect
if p.trial.stimulus.range_mean >= p.trial.stimulus.range_var
    if any(strcmp(p.trial.stimulus.step,'contrast'))
        p.trial.stimulus.rangeI = 0;
    else
...
p.conditions{p.trial.pldaps.iTrial}.pretime_final = p.trial.stimulus.pretime_final;
...
if isempty(intersect(intersect(intersect(find(p.trialMem.stats.val(2,:) == p.trial.stimulus.(p.trialMem.stats.cond{2})),find(p.trialMem.stats.val(3,:) == p.trial.stimulus.(p.trialMem.stats.cond{3}))),find(p.trialMem.stats.val(4,:) == p.trial.stimulus.(p.trialMem.stats.cond{4}))),find(p.trialMem.stats.val(5,:) == p.trial.stimulus.(p.trialMem.stats.cond{5}))))
    p.trialMem.stable(2,:) = [p.trial.stimulus.(p.trialMem.stats.cond{2}) p.trial.stimulus.(p.trialMem.stats.cond{2})];
    p.trialMem.stats.val = horzcat(p.trialMem.stats.val,vertcat(p.trialMem.stable,[p.trial.stimulus.(p.trialMem.stats.cond{3}) p.trial.stimulus.(p.trialMem.stats.cond{3})],[p.trial.stimulus.(p.trialMem.stats.cond{4}) p.trial.stimulus.(p.trialMem.stats.cond{4})],[p.trial.stimulus.(p.trialMem.stats.cond{5}) p.trial.stimulus.(p.trialMem.stats.cond{5})])); % add the adjusted parameter
    [p.trialMem.stats.val,rankind]  = (sortrows(p.trialMem.stats.val',[2,3,4,5,1],{'ascend' 'ascend' 'descend' 'ascend' 'ascend'})); % rank the adjusted parameter
...
% if isempty(intersect(find(p.trialMem.stats.val(3,:) == p.trial.stimulus.(p.trialMem.stats.cond{3})),find(p.trialMem.stats.val(4,:) == p.trial.stimulus.(p.trialMem.stats.cond{4}))))
%     p.trialMem.stats.val = horzcat(p.trialMem.stats.val,vertcat(p.trialMem.stable,[p.trial.stimulus.(p.trialMem.stats.cond{3}) p.trial.stimulus.(p.trialMem.stats.cond{3})],[p.trial.stimulus.(p.trialMem.stats.cond{4}) p.trial.stimulus.(p.trialMem.stats.cond{4})],[p.trial.stimulus.(p.trialMem.stats.cond{5}) p.trial.stimulus.(p.trialMem.stats.cond{5})])); % add the adjusted parameter
%     [p.trialMem.stats.val,rankind]  = (sortrows(p.trialMem.stats.val',[3,4,5,2,1],{'ascend' 'descend' 'ascend' 'ascend' 'ascend'})); % rank the adjusted parameter
%     p.trialMem.stats.val = p.trialMem.stats.val';
%     rankind = rankind';
%     p.trialMem.stats.count.Ntrial = horzcat([p.trialMem.stats.count.Ntrial zeros(1:2)]);
%     p.trialMem.stats.count.Ntrial = p.trialMem.stats.count.Ntrial(rankind);
...
distance_from_center_w = 35.5/4; % the distance from the grating center to the monitor center, in cm. This code makes the distance the same for monitors with different sizes
jitter_from_edge_w = (p.trial.display.widthcm - distance_from_center_w * 4)/2/2;
jitter_from_edge_px = jitter_from_edge_w * p.trial.display.w2px(1);
% if p.trial.stimulus.fullField 
    switch p.trial.stimulus.LeftRight
        case 1
             p.trial.gratPosC = [(0 + p.trial.stimulus.locationjitter) 0 (p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter) p.trial.display.screenSize(4)]; % change to your resolution - SZ
             p.trial.gratPosI = [(p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter) 0 (p.trial.display.screenSize(3) + p.trial.stimulus.locationjitter) p.trial.display.screenSize(4)];
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf);
             p.trial.jitter_from_edge_px = -jitter_from_edge_px;
        case 2
             p.trial.gratPosC = [(p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter) 0 (p.trial.display.screenSize(3) + p.trial.stimulus.locationjitter) p.trial.display.screenSize(4)];
             p.trial.gratPosI = [(0 + p.trial.stimulus.locationjitter) 0 (p.trial.display.screenSize(3)/2 + p.trial.stimulus.locationjitter) p.trial.display.screenSize(4)];
            % p.trial.stimulus.sf = (p.conditions{p.trial.pldaps.iTrial}.sf)./2;
             p.trial.jitter_from_edge_px = jitter_from_edge_px;
    end
% else
...
%     DegPerCyc = 1/p.trial.stimulus.sf;
    ...
    x=linspace(-(p.trial.display.dWidth/4),p.trial.display.dWidth/4,p.trial.display.pWidth/2);%-p.trial.stimulus.shift(p.trial.side); 
    y=linspace(-(p.trial.display.dHeight/2),p.trial.display.dHeight/2,p.trial.display.pHeight/2);
    [x,y] = meshgrid(x,y);
    ...
    % GRATING
    sdom=sdom*p.trial.stimulus.sf_final*2*pi;
    ...
    if p.conditions{p.trial.pldaps.iTrial}.centralflash % central flash
        ...
        p.trial.gratPosCentral = [p.trial.stimulus.locationjitter(1) 0 (p.trial.display.screenSize(3)+p.trial.stimulus.locationjitter(1)) p.trial.display.screenSize(4)]; %p.trial.display.screenSize;
    ...
    p.trial.stimulus.rangeI_M = (max(gratingI,[],'all') - min(gratingI,[],'all'))./(max(gratingI,[],'all') + min(gratingI,[],'all'));
...
if p.trial.pldaps.iTrial >= 14
    ...
    % increase pretime if performance in last 14 trials lower than 70%

    % automatically update reward difference to remove bias
    if ~isfield(p.trial.stimulus, 'block') || p.trial.stimulus.block ~= 1
        % Decrease reward difference
        if leftcorrectrate14 - rightcorrectrate14 < 0.2 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) > 2*p.trialMem.amountDelta
        ...
        elseif leftcorrectrate14 - rightcorrectrate14 > -0.3 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) < -4*p.trialMem.amountDelta
            pds.behavior.reward.updateAmount(p,3);
            pds.behavior.reward.updateAmount(p,6);
        % Increae reward difference, but prevent the reward from being too low
        elseif leftcorrectrate14 - rightcorrectrate14 >= 0.125 && leftcorrectrate14 - rightcorrectrate14 < 0.25 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) < 2*p.trialMem.amountDelta
            pds.behavior.reward.updateAmount(p,3);
            pds.behavior.reward.updateAmount(p,6);
        elseif leftcorrectrate14 - rightcorrectrate14 <= -0.125 && leftcorrectrate14 - rightcorrectrate14 > -0.25 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) > -2*p.trialMem.amountDelta
            pds.behavior.reward.updateAmount(p,4);
            pds.behavior.reward.updateAmount(p,5);
        elseif leftcorrectrate14 - rightcorrectrate14 <= -0.25 &&  p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) > -4*p.trialMem.amountDelta && totalcorrectrate14<0.85 && totalcorrectrate14>=0.6 % do not increase diff to confuse animal if performance is great or poor
            pds.behavior.reward.updateAmount(p,4);
            pds.behavior.reward.updateAmount(p,5);
        elseif leftcorrectrate14 - rightcorrectrate14 <= -0.375 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) < 2*p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) && totalcorrectrate14<0.85 && totalcorrectrate14>=0.6 % do not increase diff to confuse animal if performance is great or poor
            pds.behavior.reward.updateAmount(p,4);
            pds.behavior.reward.updateAmount(p,5);
        elseif leftcorrectrate14 - rightcorrectrate14 >= 0.25 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && (p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT)-p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT)) < 4*p.trialMem.amountDelta && totalcorrectrate14<0.85 && totalcorrectrate14>=0.6 % do not increase diff to confuse animal if performance is great or poor
            pds.behavior.reward.updateAmount(p,3);
            pds.behavior.reward.updateAmount(p,6);
        elseif leftcorrectrate14 - rightcorrectrate14 >= 0.375 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) >= 0.15 && p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT) < 2*p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.LEFT) && totalcorrectrate14<0.85 && totalcorrectrate14>=0.6 % do not increase diff to confuse animal if performance is great or poor
            pds.behavior.reward.updateAmount(p,3);
            pds.behavior.reward.updateAmount(p,6);
        end
    end
    
% SZ: increase reward after trial 80
if all(isfield(p.trial.behavior.reward,{'increaseSince','increaseTimes'})) && p.trial.pldaps.iTrial == p.trial.behavior.reward.increaseSince
    if p.trial.behavior.reward.increaseTimes == 3
        ...
        pds.behavior.reward.updateAmount(p,6);
        pds.behavior.reward.updateAmount(p,6);
        pds.behavior.reward.updateAmount(p,2); % added since 7/26/2023
...
%show parameter
    pds.behavior.parameter.showParameter(p) %,{'RangeC';'RangeL'})
%show stats
