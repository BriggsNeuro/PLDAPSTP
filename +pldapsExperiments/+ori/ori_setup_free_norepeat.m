function p=ori_setup_free_norepeat(p)
%experiment setup file for a simple test experiment
% SZ: traing stage 1 - get reward from all ports in any sequence, modified from ori_setup_free
%% basic definitions
p = pdsDefaultTrialStructureNL(p); 

%% set the trial function: the function that gets called for each frame state
p.trial.pldaps.trialFunction='pldapsExperiments.ori.oritrial_free_norepeat';
...
for i = 1:length(p.defaultParameters.stimulus.fullField)
    ...
    % predictable
    c{i} = generateCondList(cond(i),side,p.defaultParameters.stimulus.runtype,p.defaultParameters.stimulus.nrBlocks, p.defaultParameters.stimulus.nrReps);
end
