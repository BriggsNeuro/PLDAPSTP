function oritrial_free_norepeat(p,state)
% SZ: traing stage 1 - get reward from all ports in any sequence, modified from oritrial_free
...
%             pds.behavior.reward.give(p,amount,p.trial.behavior.reward.channel.START);
   ...
   %     end
    ...
    case p.trial.stimulus.states.STIMON %stimulus shown; port selected in response
        ...
        if p.trial.stimulus.midpointIR & find(p.trial.ports.status==1)==p.trial.stimulus.port.MIDDLE %check IR beam for box midpoint
            p.trial.stimulus.midpointCrossed = 1;
        end
        if ~isempty(p.data)
            if ismember(activePort, [p.trial.stimulus.port.LEFT p.trial.stimulus.port.RIGHT p.trial.stimulus.port.START])  & ~isequal(p.data{1,end}.stimulus.respTrial, p.trial.ports.status)
                %note time
                ...
                previousportstatus = p.trial.ports.status;

                %check whether correct port chosen
                % correct=checkPortChoice(activePort,p);


                 if activePort==p.trial.stimulus.port.LEFT & length(activePort)==1 %start port activated
                        %deliver reward
                ...
                if activePort==p.trial.stimulus.port.RIGHT & length(activePort)==1 %start port activated
                            %deliver reward
            ...
            if ismember(activePort, [p.trial.stimulus.port.LEFT p.trial.stimulus.port.RIGHT p.trial.stimulus.port.START])
                %note time
                p.trial.stimulus.timeTrialFirstResp = p.trial.ttime;
                p.trial.stimulus.frameTrialFirstResp = p.trial.iFrame;
...
%             %check whether any port chosen
%             if ismember(activePort, [p.trial.stimulus.port.MIDDLE p.trial.stimulus.port.LEFT p.trial.stimulus.port.RIGHT p.trial.stimulus.port.START]) && p.trial.stimulus.respTrial ~=p.trial.ports.status
%                 %check whether correct port chosen
%                 correct=checkPortChoice(activePort,p);                
...
%     DegPerCyc = 1/p.trial.stimulus.sf;
    ...
    p.trial.gratPos = [0 0 1024 768]; % change to your resolution - SZ
